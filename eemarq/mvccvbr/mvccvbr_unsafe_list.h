#ifndef MVCCVBR_UNSAFE_LIST_H_
#define MVCCVBR_UNSAFE_LIST_H_
...
private:
    ...
    inline void initDirectCTSNode(DirectCTSNode<K,V> *node, K key, V value, DirectCTSNode<K,V> *next) {
      readGlobalReclamationEpoch();
      uint64_t shiftedReclamationEpoch = (currEpoch << SNAPSHOT_EPOCH_BITS);
      node->ts.store(shiftedReclamationEpoch, std::memory_order_acq_rel);
...
public:
    ...
    mvccvbr_unsafe_list(int numProcesses, const K key_min, const K key_max, const V no_value) : KEY_MIN{key_min}, KEY_MAX{key_max}, NO_VALUE{no_value} {
        head = nullptr;
        ...
        initDirectCTSNode(tail, KEY_MAX, NO_VALUE, nullptr);
        
                       
        head = (DirectCTSNode<K,V> *)localAllocator->alloc();
        initDirectCTSNode(head, KEY_MIN, NO_VALUE, tail);
#if defined(MVCC_VBR_SKIPLIST)        
  ...
  private:
    ...
    DirectCTSNode<K,V> *trim(DirectCTSNode<K,V> *pred, DirectCTSNode<K,V> *curr, uint64_t predVersion) {
      ...
      initDirectCTSNode(succTag, succKey, succ->value, succNext);
  ...
  public:
    ...
    V insert(const int tid, const K& key, const V& value) { 
      ...
      while (true) {
        ...
        initDirectCTSNode(newNode, key, value, curr); 
    ...
    //intptr_t rangeQuery(intptr_t low, intptr_t high, int tid) {      
      ...
      uint64_t predVersion, rangeQueryEpoch, predTS, currTS;
      int count;
      ...
      while (true) {
        ...
        do {
          curr = getDirectCTSNode(pred->next);
          if (curr == nullptr) {
            currKey = lo;
            break;
          }
          ...
          if (pred->getTS() != predTS || isValidTS(currTS) == false) {
            currKey = lo;
            break;
          }         
        ...
        if (curr == nullptr || pred->getTS() != predTS || isValidTS(currTS) == false) {
