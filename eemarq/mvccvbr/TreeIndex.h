...
class TreeIndex
{
  ...
  public:
    ...
    bool insert(DirectCTSNode<K,V> *n, uint64_t nTS)
    {
        ...
        if (n->key % INSERT_FREQ == 0) return false;
    ...
    bool remove(K key)
    {
        ...
        if (key % INSERT_FREQ == 0) return false;
    ...
    int getInitialNumCaches() {
      return globalTreeIndexAllocator->getInitialNumCaches();
    }
